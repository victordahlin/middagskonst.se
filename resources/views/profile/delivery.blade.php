  ...
  <div class="col-md-12 col-xs-12">
    ...
    <table class="table table-striped">
      <tbody>
        <tr>
          <td>
            <input type="radio" class="col-sm-8" name="subscription" value="off" @if($data['interval'] == 'off')
            checked @endif >
          </td>
        ...
        <tr>
          <td><input type="radio" class="col-sm-8" name="subscription"
            value="eachWeek" @if($data['interval'] == 'eachWeek') checked @endif ></td>
            <td class="col-md-10"><b>Varje vecka</b></td>
            <td></td>
          </tr>
          <tr>
            <td><input type="radio" class="col-sm-8" name="subscription"
              value="everySecondWeek" @if($data['interval'] == 'everySecondWeek') checked @endif ></td>
    ...
    <div class="row">
      <div class="col-md-12 col-xs-12">
        ...
        <table class="table table-responsive">
          <tbody>
            @for ($i = 0; $i < 5; $i++)
            @if($data['fiveWeeks'][$i] >= $data['startDate'] )
            <tr class="dateTR{{ $i }}">
              <td>
                <input type="checkbox" name="weeks[]" value="{{ $data['fiveWeeks'][$i] }}" class="col-md-3 skipDate" @if($data['skipWeeks'][$i] != "No") checked @endif>
              </td>
